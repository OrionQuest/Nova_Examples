...
class Standard_Tests: public Embedded_Deformables_Example<T,d>
{
  ...
  public:
     ...
     * example explanation:
     *
     * 1. Embedded circle stretched horizontally with linear elasticity.
     * 2. Embedded circle stretched horizontally with neo hookean model.
     ****************************/
...
//######################################################################
    ...
    {return simulation_mesh->Get_Element(element).First_Two_Barycentric_Coordinates(X);}
//######################################################################
    void Set_Kinematic_Positions(const T time,Array<TV>& X) const override
    {
        if(test_number==1 || test_number==2){
            for(size_t i=0;i<indices_of_fixed_particles(0).size();++i) X(indices_of_fixed_particles(0)(i))=rest_position_of_fixed_particles(0)(i)-TV({edge_velocity*time,0});
            for(size_t i=0;i<indices_of_fixed_particles(1).size();++i) X(indices_of_fixed_particles(1)(i))=rest_position_of_fixed_particles(1)(i)+TV({edge_velocity*time,0});}
    }
//######################################################################
    virtual void Set_Kinematic_Velocities(const T time,Array<TV>& V) const override
    {
        if(test_number==1 || test_number==2){
            for(size_t i=0;i<indices_of_fixed_particles(0).size();++i) V(indices_of_fixed_particles(0)(i))=TV({-edge_velocity,0});
            for(size_t i=0;i<indices_of_fixed_particles(1).size();++i) V(indices_of_fixed_particles(1)(i))=TV({edge_velocity,0});}
    }
//######################################################################
