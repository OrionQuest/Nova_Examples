  ...
  public:
        ...
        :simulation_mesh(simulation_mesh_input)
    {
        ...
        for(size_t i=0;i<simulation_mesh->elements.size();++i){
            Matrix<T,d> Dm=Get_Shape_Matrix(simulation_mesh->points,i);
    ...
    Matrix<T,d> Get_Shape_Matrix(const Array<TV>& X,const int index) const
    {
        Vector<TV,d> columns;
        const INDEX& e=simulation_mesh->elements(index);
        for(int i=1;i<d+1;++i) columns(i-1)=X(e(i))-X(e(0));
        return Matrix<T,d>(columns);
    }

    void Distribute_To_Simulation_Particles(Array<TV>& force,const Matrix<T,d>& G,const int index) const
    {
