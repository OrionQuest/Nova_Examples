...
namespace Nova {
    ...
    public:
        ...
        virtual void RenderText( std::string text, float scale, float x, float y, float width, float height ){
            ...
            int charPos = 0; // Char index on given line
            for (c = text.begin(); c != text.end(); c++)
                {
                    /* Handle control characters */
                    if (*c == '\n') {
                        float offsetY = 4.0;
                        y += spacing * (this->Characters['H'].Bearing.y + offsetY) * scale;
                        x = startX;
                        charPos = 0;
                        continue;
                    }
                        ...
                        float numSpacePerTab = 8 - (charPos % 8);
                        x += numSpacePerTab * (this->Characters[' '].Advance >> 6) * scale;
                        charPos += numSpacePerTab;
                        continue;
                    }
